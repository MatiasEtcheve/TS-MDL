...
def _multivariate_OMP(signal, dictionary, n_nonzero_coefs=None, verbose=False):
    ...
    Ainv = np.zeros((n_nonzero_coefs, n_nonzero_coefs), np.float)
...
def multivariate_sparse_encode(
    ...
    X = array3d(X)
    n_samples, n_features, n_dims = X.shape
    if isinstance(dictionary, MultivariateDictLearning):
...
def multivariate_dict_learning(
    ...
    if dict_init is not None:
        ...
        max_offset = n_features - np.max(k_len)
        ...
        if n_samples>=n_kernels:
            ...
            ind_kernels = []
            for ker in range(n_kernels):
                ind_kernels = ind_kernels + list(random_state.permutation(1))
            ind_kernels = np.array(ind_kernels)[:n_kernels]
        dictionary = [X[p[0], p[1] : p[1] + k_len[k], :] for k,p in enumerate(zip(ind_kernels, offset))]
        dictionary = _normalize(dictionary)
